...
<Tabs>
  <TabItem value="mysql84" label="MySQL 8.4 (Default)" default>
    Run in Terminal:
    ```bash
  ...
  <TabItem value="mysql80" label="MySQL 8.0" >
    Run in Terminal:
    ```bash
    cd /usr/local/src && rm -f cpguardx.sh && curl -o cpguardx.sh -L https://downloads.opsshield.com/cpguard/cpguardx.sh && DB_ENGINE='MYSQL_8.0' bash cpguardx.sh LICENCE-KEY
    ```
  </TabItem>
